...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if sum(relevant_chunks) == 0:
            return 0.0

        relevant_count = 0
        ap_sum = 0.0
        for k, is_rel in enumerate(relevant_chunks, start=1):
            if is_rel:
                relevant_count += 1
                precision_at_k = relevant_count / k
                ap_sum += precision_at_k

        return ap_sum / sum(relevant_chunks)
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_faithfulness": faithfulness / total,
            "avg_relevance": relevance / total,
            "avg_completeness": completeness / total,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def get_avgs(res):
            ...
            tot = len(res)
            return (
                sum(r.faithfulness for r in res) / tot,
                sum(r.relevance for r in res) / tot,
                sum(r.completeness for r in res) / tot
            )
